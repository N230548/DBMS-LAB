...
order by total_income desc
limit 1;
 select category_id, avg(amount) as average_income from income_record group by category_id order by average_income desc
limit 1;
select category_id, count(*) as total_records from income_record group by category_id having count(*) > 2;
select year_id, sum(amount) as total_income from income_record group by year_id having sum(amount) > 1000000;
select category_id,count(*) as number_of_records,sum(amount) as total_income,avg(amount) as average_income,max(amount) as highest_income,min(amount) as lowest_income
